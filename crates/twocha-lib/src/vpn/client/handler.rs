...
pub fn run(config_path: &str, quiet: bool) -> Result<()> {
    ...
    while RUNNING.load(Ordering::SeqCst) {
        // Handle TUN reads
        handle_tun_read_windows(&mut tun, &mut tun_buffer, &mut tunnel, &mut server_peer)?;

        // Handle UDP reads
        handle_udp_read_windows(&mut tunnel, &mut tun, &mut server_peer, &cipher)?;
